...
Button::Button(IDirect3DDevice9* device, const char* label,
    ...
    : device(device), label(label),
      ...
      hovered(false), selected(false), enabled(true), prevMouseDown(false)
{
    rect.left = x;
    rect.top = y;
    rect.right = x + width;
...
void Button::SetColours(D3DCOLOR fill, D3DCOLOR fillActive, D3DCOLOR border, D3DCOLOR text)
{
    colFill = fill;
    colFillActive = fillActive;
    colBorder = border;
    colText = text;
}
...
{
    hovered = enabled && IsHovered(mouseX, mouseY);
...
void Button::Render(LPD3DXSPRITE brush)
{
    if (device == NULL) return;

    // Fill: clear just this rectangle of the back buffer to a solid colour
    D3DRECT area = { rect.left, rect.top, rect.right, rect.bottom };
    D3DCOLOR fill = !enabled ? colFillDisabled
                             : ((hovered || selected) ? colFillActive : colFill);
