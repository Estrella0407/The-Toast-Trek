...
// A bar whose fill levels are pre-drawn frames stacked top-to-bottom in one
// PNG (frame 0 = empty, last frame = full, every frame the same size). The
// game just picks the frame that matches the current value, so the fill
// always looks hand-drawn instead of a stretched flat quad.
class FrameBar {
public:
    // `path` is the vertical strip; frameW x frameH is one frame; the strip
    // is expected to be frameW x (frameH * frameCount).
    ...
    // Draw frame `index` (clamped to [0, frameCount-1]) with its top-left at
    // (x, y), uniformly scaled. `tint` multiplies the frame's colours.
